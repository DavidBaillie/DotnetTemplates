    ...
    : IntegrationTestBase
{
    ...
    [Test]
    public async Task Invoke_WithShorterKey_ReturnsUnauthorized()
    {
        // Arrange - Tests constant-time comparison with shorter key
        using var request = new HttpRequestMessage(HttpMethod.Get, TEST_ENDPOINT);
        request.Headers.Add(API_KEY_HEADER_NAME, VALID_API_KEY[..^5]);
    ...
    [Test]
    public async Task Invoke_WithWhitespaceOnlyKey_ReturnsUnauthorized()
    {
        // Arrange
        using var request = new HttpRequestMessage(HttpMethod.Get, TEST_ENDPOINT);
        request.Headers.Add(API_KEY_HEADER_NAME, "   ");
    ...
    [Test]
    public async Task Invoke_WithKeyContainingLeadingWhitespace_ReturnsUnauthorized()
    {
        // Arrange - Leading whitespace should not match
        using var request = new HttpRequestMessage(HttpMethod.Get, TEST_ENDPOINT);
        request.Headers.Add(API_KEY_HEADER_NAME, " " + VALID_API_KEY);
    ...
    [Test]
    public async Task Invoke_WithKeyContainingTrailingWhitespace_ReturnsUnauthorized()
    {
        // Arrange - Trailing whitespace should not match
        using var request = new HttpRequestMessage(HttpMethod.Get, TEST_ENDPOINT);
        request.Headers.Add(API_KEY_HEADER_NAME, VALID_API_KEY + " ");
